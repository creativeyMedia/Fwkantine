        ...
        if orders:  # Only include dates with orders
            ...
            for order in orders:
                if order.get("is_sponsored") and not order.get("is_sponsor_order"):
                    ...
                    if order.get("sponsored_meal_type") == "breakfast":
                        ...
                        remaining_cost = max(0, breakfast_cost - sponsored_breakfast_cost)
                        total_amount += remaining_cost
                    ...
                    # For sponsor orders, count the full total_price (sponsor pays for everyone)
                    total_amount += order.get("total_price", 0)
                else:
            ...
            for employee_key in employee_orders.keys():
                ...
                sponsored_orders = await db.orders.find({
                    "department_id": department_id,
                    "sponsored_by_employee_id": {"$exists": True},
                    "sponsored_by_name": employee_name,
                    "timestamp": {
                        "$gte": start_of_day_utc.isoformat(),
                        "$lte": end_of_day_utc.isoformat()
                    }
...
async def sponsor_meal(meal_data: dict):
    ...
    try:
        ...
        for order_update in other_order_updates:
            ...
            updated_order = await db.orders.find_one({"id": order_update["id"]})
            if updated_order:
                print(f"✅ DEBUG: Order {order_update['id']} now has is_sponsored: {updated_order.get('is_sponsored')}, sponsored_meal_type: {updated_order.get('sponsored_meal_type')}")
            else:
                print(f"❌ DEBUG: Could not find updated order {order_update['id']}")
